...
function LogDetail({ log, now }: { log: RelayLogOverview; now: number }) {
    ...
    const requestBody = useLogRequestBody(
        log.id,
        log.started_at,
        detailReady && leftTab === "request",
    );
    const responseBody = useLogResponseBody(
        log.id,
        log.started_at,
        detailReady && log.status === "success",
    );
    const { data: activeGroup } = useGroup(
        log.group_id,
        detailReady,
        detailReady,
    );
    ...
    const showRounds =
        log.status === "running" || (requestFailed && rounds.length > 0);
    const isWaitingForSelection =
        log.status === "running" &&
        !log.sending &&
        activeGroup?.mode === "manual" &&
        activeGroup.runtime.current_item_id === 0; // isWaitingForSelection 表示手动模式请求正等待选择渠道。
    ...
    const roundKey =
        log.round === 0
            ? ""
            : `${log.round}:${log.target_channel}:${log.sending}:${errorText}`;
    if (roundKey !== "" && roundKey !== observedRoundKey) {
        setObservedRoundKey(roundKey);
        setRounds((current) => {
            if (
                !log.sending &&
                current.every((item) => item.round !== log.round)
            )
                return current;
            return [
                {
    ...
    return (
        <MorphingDialogContent className="relative w-[calc(100vw-2rem)] md:w-[80vw] bg-card text-card-foreground px-6 py-4 rounded-3xl h-[calc(100vh-2rem)] flex flex-col overflow-hidden">
            ...
            <MorphingDialogTitle className="flex flex-wrap items-center gap-2 pr-8 mb-3 text-sm">
                ...
                <span className="text-xs text-muted-foreground/70">
                    {PROTOCOL_LABELS[log.target_protocol] ?? "-"}
                </span>
                <Badge
                    variant="secondary"
                    ...
                    <div className="flex flex-col rounded-2xl border border-border bg-muted/30 overflow-hidden min-h-0">
                        <div className="flex h-10 shrink-0 items-center gap-2 border-b border-border bg-muted/50 pl-1 pr-3 md:pr-4">
                            <Tabs
                                value={leftTab}
                                onValueChange={(value) =>
                                    setLeftTab(value as "request" | "group")
                                }
                            >
                                <TabsList variant="text" className="p-0">
                                    ...
                                    <span
                                        aria-hidden="true"
                                        className="mx-1 inline-flex h-full -translate-y-px items-center text-sm font-medium leading-none text-muted-foreground/50"
                                    >
                                        /
                                    </span>
                                    <TabsTrigger
                            ...
                            {leftTab === "request" && (
                                ...
                                >
                                    {(
                                        ...
                                        (log.usage.prompt_tokens_details
                                            ?.cached_tokens ?? 0)
                                    ).toLocaleString()}{" "}
                                    {t("tokens")}
                                </Badge>
                                            ...
                                            <button
                                                ...
                                                onClick={async () => {
                                                    ...
                                                    const isCurrent =
                                                        ...
                                                        item.id;
                                                    try {
                                                        await updateActiveItem.mutateAsync(
                                                            {
                                                                id: activeGroup.id,
                                                                active_item_id:
                                                                    isCurrent
                                            ...
                                            >
                                                ...
                                                <span className="min-w-0 flex-1">
                                                    ...
                                                    <span className="block truncate text-[11px] text-muted-foreground">
                                                        {item.key_name
                                                            ? `${item.channel_name} · ${item.key_name}`
                                                            : item.channel_name}
                                                    </span>
                                                </span>
                                                <MemberStatus
            ...
            >
                <div
                    className={cn(
                        ...
                        requestFailed ? "items-start" : "items-center",
                    )}
                >
                    <Icon
                        aria-hidden="true"
                    ...
                    <div className="min-w-0 flex flex-col gap-3">
                        ...
                        {requestFailed && errorText && (
                            <div className="p-2.5 rounded-xl bg-destructive/10 border border-destructive/20 overflow-hidden">
                                <p className="text-xs text-destructive line-clamp-2 whitespace-pre-line">
                                    {errorText}
                                </p>
                            </div>
                        )}
...
export const LogCard = memo(function LogCard({
    log,
}: {
    log: RelayLogOverview;
}) {
